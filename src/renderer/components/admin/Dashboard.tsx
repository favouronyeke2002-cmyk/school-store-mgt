...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top stat strip — 5 key financial metrics */}
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
        <div className="grid grid-cols-5 divide-x min-w-[480px]">
          {[
            { label: 'Total Income', value: fmt(stats?.totalIncome || 0), color: 'text-gray-900' },
            { label: 'Total Expenses', value: fmt(stats?.totalExpenses || 0), color: 'text-danger-700' },
            { label: 'Net Balance', value: fmt(stats?.netBalance || 0), color: (stats?.netBalance || 0) >= 0 ? 'text-success-700' : 'text-danger-700' },
            { label: 'School Revenue', value: fmt(stats?.schoolRevenue || 0), color: 'text-primary-700' },
            { label: 'Store Revenue', value: fmt(stats?.storeRevenue || 0), color: 'text-gray-900' },
          ].map((item) => (
