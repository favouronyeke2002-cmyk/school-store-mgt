...
const StudentHistoryDrawer: React.FC<{
  target: { id: string; name: string; class: string; isApplicant?: boolean; current_fees_owed?: number } | null;
  settings: any;
  onClose: () => void;
      ...
      try {
        ...
        if (target.isApplicant) {
          ...
          try {
            ...
            const unpaid = (payments || []).filter((p: any) => p.status !== 'completed');
            owed = unpaid.reduce((s: number, p: any) => s + (Number(p.amount) - Number(p.amount_paid || 0)), 0);
          } catch {}
        } else {
  ...
  const totalPaid = validTxns.reduce((sum, t) => sum + Number(t.amount_paid), 0);

  const handleReprint = (t: any) => {
    const isFees = t.type === 'FEES_CASH_COLLECTION';
    const isRegistration = t.type === 'BUNDLE_PURCHASE' || t.type === 'ACCEPTANCE_FEE';
    const printTxn = {
      ...t,
    ...
    printReceipt(buildReceiptHtml(settings, printTxn, Number(t.amount_paid), t.items || [], isFees, isRegistration));
  };
      ...
      <div className="w-full max-w-xl bg-white h-full shadow-2xl flex flex-col">
        ...
        <div className="p-5 border-b flex items-center justify-between bg-slate-900 text-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300">{target.class} &bull; {target.isApplicant ? `Applicant #${target.id}` : target.id}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-slate-800 text-slate-300 hover:text-white">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 bg-slate-50 border-b shrink-0 grid grid-cols-2 gap-4">
          <div className="bg-white p-4 rounded-xl border border-red-100 shadow-sm">
            <div className="text-xs font-semibold uppercase text-red-600 tracking-wider mb-1">Outstanding Balance</div>
            <div className="text-2xl font-extrabold text-red-600">{fmt(balanceOwed)}</div>
            <div className="text-[11px] text-gray-400 mt-1">{balanceOwed > 0 ? 'Pending collection' : 'Fully settled'}</div>
          </div>
          <div className="bg-white p-4 rounded-xl border border-emerald-100 shadow-sm">
            <div className="text-xs font-semibold uppercase text-emerald-600 tracking-wider mb-1">Total Amount Paid</div>
            <div className="text-2xl font-extrabold text-emerald-600">{fmt(totalPaid)}</div>
            <div className="text-[11px] text-gray-400 mt-1">{validTxns.length} completed transactions</div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider">Transaction Ledger</h3>
            <span className="text-xs text-gray-500">{txns.length} total entries</span>
          </div>

          {loading ? (
            <div className="py-16 text-center text-gray-400">Loading ledger records...</div>
          ) : txns.length === 0 ? (
            ...
            <div className="space-y-3">
              {txns.map((t) => {
                ...
                const itemsSummary = itemsCount > 0
                  ? t.items.map((i: any) => `${i.item_name}${i.quantity > 1 ? ` x${i.quantity}` : ''}`).join(', ')
                  : t.fee_type_name || (t.type === 'STORE_PURCHASE' ? 'Store Purchase' : t.type === 'FEES_CASH_COLLECTION' ? 'School Fees' : t.type);

                return (
                  <div key={t.transaction_id || t.id} className={`p-4 rounded-xl border transition-all ${isVoided ? 'bg-red-50/40 border-red-200 opacity-75' : 'bg-white border-gray-200 hover:border-blue-300 shadow-sm'}`}>
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs text-gray-500 font-semibold">#{t.transaction_id || t.id}</span>
                          <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${isVoided ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>
                            {isVoided ? 'VOIDED' : 'COMPLETED'}
                          </span>
                          <span className="text-xs text-gray-400">
                            {t.payment_mode === 'POS_Transfer' ? 'POS / Transfer' : 'Cash'}
                          </span>
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        <div className={`text-base font-bold ${isVoided ? 'line-through text-gray-400' : 'text-gray-900'}`}>
                          {fmt(t.amount_paid)}
                        </div>
        ...
        <div className="p-4 border-t bg-gray-50 shrink-0">
          <button onClick={onClose} className="w-full py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-xl font-medium text-sm">
            Close Ledger
          </button>
        </div>
      </div>
...
}> = ({ txn, total, items, settings, isFees = false, isRegistration = false, onClose, onGoToFulfillment }) => {
  const handlePrint = () => printReceipt(buildReceiptHtml(settings, txn, total, items, isFees, isRegistration));
  const hasPhysicalItems = (items && items.length > 0) || isRegistration || txn?.type === 'STORE_PURCHASE' || txn?.type === 'BUNDLE_PURCHASE';
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 p-6 text-center">
        ...
        <button onClick={handlePrint} className="w-full py-3 bg-gray-800 text-white rounded-xl mb-2 font-semibold flex items-center justify-center gap-2 hover:bg-gray-900"><Printer className="w-4 h-4" /> Print Receipt (80mm)</button>
        {hasPhysicalItems && onGoToFulfillment && (
          <button onClick={onGoToFulfillment} className="w-full py-2.5 bg-primary-600 text-white rounded-xl mb-2 font-semibold flex items-center justify-center gap-2 hover:bg-primary-700 transition-all shadow-sm">
...
const CashierPOS: React.FC = () => {
  ...
  const [historyDrawerTarget, setHistoryDrawerTarget] = useState<{ id: string; name: string; class: string; isApplicant?: boolean; current_fees_owed?: number } | null>(null);
  ...
  const shiftCash = historyTxns.filter((t) => t.payment_mode === 'Cash' && t.status !== 'VOIDED').reduce((s, t) => s + Number(t.amount_paid), 0);
  const shiftPOS = historyTxns.filter((t) => t.payment_mode === 'POS_Transfer' && t.status !== 'VOIDED').reduce((s, t) => s + Number(t.amount_paid), 0);
    ...
    <div className="h-screen flex bg-gray-100 overflow-hidden">
      ...
      <aside className="w-16 bg-gray-900 flex flex-col items-center py-4 shrink-0">
        ...
        {([
          ...
          { id: 'fulfillment', icon: <Package className="w-5 h-5" />, label: 'Store Fulfillment' },
        ...
        <div className="flex flex-1 min-w-0 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="bg-white border-b px-5 py-4 shrink-0">
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border rounded-xl shadow-lg z-30">
                    <div className="max-h-60 overflow-auto">
                      {studentSuggestions.map((s) => (
                        <div key={s.student_id} className="w-full px-4 py-3 flex items-center justify-between hover:bg-gray-50 border-b last:border-0 text-left">
                          <button onClick={() => selectStudent(s)} className="flex-1 text-left min-w-0 mr-2">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                setHistoryDrawerTarget({ id: s.student_id, name: s.name, class: s.student_class, current_fees_owed: s.current_fees_owed });
                              }}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {walkInEditMode ? (
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          type="button"
                          onClick={() => setHistoryDrawerTarget({ id: String(walkInApplicant.id), name: walkInApplicant.full_name, class: walkInApplicant.proposed_class || 'Applicant', isApplicant: true })}
              ...
              {/* Selected Student Details Bar & Ledger Action */}
              {selectedStudent && (
                <div className="mt-3 flex items-center justify-between bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5">
                  <div className="min-w-0">
                    <span className="font-bold text-sm text-blue-950">{selectedStudent.name}</span>
                    <span className="text-xs text-blue-600 ml-2 font-medium">({selectedStudent.student_class} &bull; {selectedStudent.student_id})</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => setHistoryDrawerTarget({ id: selectedStudent.student_id, name: selectedStudent.name, class: selectedStudent.student_class, current_fees_owed: selectedStudent.current_fees_owed })}
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
            <div className="px-5 py-4 border-b flex items-center justify-between">
              ...
              <span className="text-sm font-bold text-gray-600">Total: {fmt(filteredHistory.filter(t => t.status !== 'VOIDED').reduce((s, t) => s + Number(t.amount_paid), 0))}</span>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredHistory.map((t) => {
                    const isVoided = t.status === 'VOIDED';
                    return (
                      <tr key={t.transaction_id} className={`border-t hover:bg-gray-50 ${isVoided ? 'opacity-60 bg-red-50/40' : ''}`}>
                        <td className="px-4 py-3 text-gray-500">{new Date(t.timestamp).toLocaleTimeString()}</td>
                        <td className="px-4 py-3 font-medium">
                          <span className={isVoided ? 'line-through text-gray-400' : ''}>{t.student_name}</span>
                          {isVoided && <span className="ml-2 px-1.5 py-0.5 rounded text-[10px] font-bold bg-danger-100 text-danger-700">VOIDED</span>}
                        </td>
                        ...
                        <td className={`px-4 py-3 text-right font-bold ${isVoided ? 'line-through text-gray-400' : ''}`}>{fmt(Number(t.amount_paid))}</td>
                        <td className="px-4 py-3 text-center">
                          {!isVoided && (
                            <button onClick={() => handleReprint(t.transaction_id)} className="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" title="Reprint Receipt"><Printer className="w-4 h-4" /></button>
                          )}
                        </td>
                      </tr>
                    );
