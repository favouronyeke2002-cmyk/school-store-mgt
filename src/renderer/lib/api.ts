...
export const studentFeeAPI = {
  ...
  async recordPayment(
    ...
    const { data: sf } = await supabase
      .from("student_fees")
      .select("amount_due, amount_paid, fee_type_id, fee_types(name)")
      .eq("id", studentFeeId)
      .single();
    ...
    // Double-entry ledger: write both debit (charge) and credit (payment)
    const feeTypeName = (sf as any).fee_types?.name || "Fee Payment";
...
export const transactionAPI = {
  ...
  async getDetails(transactionId: number) {
    const { data: items, error: itemsError } = await supabase
      .from("transaction_items")
      .select("*, inventory(item_name, selling_price)")
    ...
    let receiptItems = (items || []).map((i: any) => ({
      ...i,
    ...
    if (
      ...
      ["BUNDLE_PURCHASE", "Bundle", "ACCEPTANCE_FEE"].includes(txn?.type) &&
      txn.bundle_id
    ) {
      const { data: bundleItems, error: bundleItemsError } = await supabase
        .from("bundle_items")
        .select("quantity, inventory(item_name, selling_price)")
        .eq("bundle_id", txn.bundle_id);
      if (bundleItemsError) throw bundleItemsError;
      receiptItems = (bundleItems || []).map((item: any) => ({
        item_name: item.inventory?.item_name || "Bundle Item",
    ...
    if (receiptItems.length === 0 && txn) {
      const isTuitionPayment = ["FEES_CASH_COLLECTION", "FEE_PAYMENT"].includes(
        txn.type,
      );
      receiptItems = [
        {
          ...
          is_fee_payment: isTuitionPayment,
        },
      ];
    }
  ...
  async void(transactionId: number) {
    ...
    if (
      ...
      try {
        ...
        // Student or fee record may have been deleted — void still succeeds
      }
    }
    ...
    // 5. Reverse ledger entries (double-entry accounting)
    try {
      await ledgerAPI.reverseForTransaction(transactionId);
    } catch (e) {
  ...
  async createPurchase(
    ...
    // Track ALL items in student_book_issuances for fulfillment tracking.
    // In-stock items are marked stock_deducted=true and status=assigned.
    // Out-of-stock items are stock_deducted=false and status=unassigned.
    if (cart.length > 0 && studentId) {
      const allIssuanceRows = cart.map((i) => {
        const available = liveStock[i.item_id] ?? 0;
        ...
        return {
          ...
          item_name: i.item_name,
          bundle_name: "Store Purchase",
          quantity: i.quantity,
          status: wasInStock ? "assigned" : "unassigned",
  ...
  async getForApplicant(applicantId: number) {
    const { data: txns, error } = await supabase
      .from("transactions")
      .select("*, fee_types(name)")
...
export const adminAPI = {
  async getStats() {
    ...
    // ── Aggregate metrics ─────────────────────────────────────────────────────
    const totalIncome = schoolRevenue + storeRevenue;
...
function itemFitsClass(
  ...
  if (normalizedTags.some((tag: string) => tag === "ALL" || tag === "GENERAL"))
    return true;
  const studentEligibilityTags = [studentClass, ...studentTags]
    .filter(Boolean)
    .map(normalizeClassTag)
    .filter(Boolean);
  if (studentEligibilityTags.length === 0) return false;
  return normalizedTags.some((itemTag: string) =>
    studentEligibilityTags.some(
      (studentTag: string) =>
        studentTag === itemTag ||
        studentTag.startsWith(itemTag) ||
        itemTag.startsWith(studentTag),
    ),
  );
}
...
export const bundlePaymentAPI = {
  ...
  async processBundlePayment(params: {
    ...
    const applicableBundleItems = requestedItems.filter((item: any) =>
      itemFitsClass(item, targetClass, studentTags),
    );
    ...
    // Double-entry ledger for bundle/acceptance fee payment
    if (txn.student_id || txn.applicant_id) {
      try {
        await ledgerAPI.recordDoubleEntry({
          studentId: txn.student_id || `applicant_${applicantId}`,
          transactionId: txn.transaction_id,
          feeTypeName: bundle.name || "Acceptance Fee",
          amount: amountPaid,
  ...
  async processDirectRegistrationPayment(params: {
    ...
    if (finalBundleItems.length > 0) {
      ...
      const allIssuanceRows = finalBundleItems.map((item) => {
        ...
        return {
          ...
          book_name: item.item_name,
          item_name: item.item_name,
          bundle_name: `Registration — ${categoryGroup} ${studentStatus}`,
          quantity: item.quantity,
          status: wasInStock ? "assigned" : "unassigned",
          stock_deducted: wasInStock,
        };
    ...
    // Double-entry ledger for registration payment
    try {
      await ledgerAPI.recordDoubleEntry({
        studentId: `applicant_${applicantId}`,
        transactionId: txn.transaction_id,
        feeTypeName: `Registration — ${categoryGroup} ${studentStatus}`,
        amount: amount,
        paymentMode: paymentMode,
      });
    } catch (e) {
...
async function enrichIssuanceRows(rawRows: any[]): Promise<any[]> {
  if (!rawRows || !rawRows.length) return [];

  // 1. Gather IDs that need manual lookup if direct relation join failed
  ...
  return rawRows.map((row: any) => {
    ...
    } else if (row.applicant_id && applicantMap.has(Number(row.applicant_id))) {
      const a = applicantMap.get(Number(row.applicant_id));
      studentName = `${a.first_name || ""} ${a.last_name || ""}`.trim();
      studentClass = a.proposed_class || "New Admission";
    }
  ...
  async getAllPending() {
    ...
    try {
      ...
      try {
        ...
        console.error("getAllPending fallback error:", err);
        return [];
      }
    }
  ...
  async getAll() {
    ...
    try {
      ...
      try {
        ...
        console.error("getAll fallback error:", err);
        return [];
      }
    }
...
export const ledgerAPI = {
  ...
  async getStatement(studentId: string) {
    const { data, error } = await supabase
      .from("ledger_entries")
      .select("*")
      .eq("student_id", studentId)
  ...
  async getAllStudentSummaries() {
    ...
    return students.map((s: any) => {
      ...
      return {
        ...
        balance_due: sum.totalBilled - sum.totalPaid,
      };
    });
  },

  async reverseForTransaction(transactionId: number) {
    const { error } = await supabase
      .from("ledger_entries")
      .delete()
      .eq("transaction_id", transactionId);
